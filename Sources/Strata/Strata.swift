...
struct NodeView:View{
 let node: MindNode
 @ObservedObject var store:MindMapStore;@Binding var editing:UUID?;@Binding var draft:String
 ...
 @ViewBuilder var nodeLabel:some View{Group{if editing==node.id{TextField("节点",text:$draft,onCommit:{store.rename(draft);editing=nil}).textFieldStyle(.plain).padding(10).frame(width:160).focused($focused).onAppear{draft=node.title;focused=true}.onKeyPress(.escape){editing=nil;return .handled}}else{Text(node.title).padding(10)}}.background(node.id==store.selectedID ? Color.accentColor.opacity(0.3):Color.secondary.opacity(0.15)).clipShape(RoundedRectangle(cornerRadius:8)).contentShape(Rectangle()).focusable().onTapGesture{store.selectedID=node.id}.onTapGesture(count:2){store.selectedID=node.id;editing=node.id;draft=node.title}.onKeyPress(.return){guard editing==nil else{return .ignored};store.addSibling();return .handled}.onKeyPress(.tab){guard editing==nil else{return .ignored};store.addChild();return .handled}}
}
